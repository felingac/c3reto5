...
function pintarRespuestaCategory(items){
   ...
   let myTable = "<table border=1>";
    for(i=0; i<items.length; i++) { 
        ...
        myTable += "<td> <button onclick='borrarElementoCategory(" + items[i].id + ")'> Borrar </button></td>";
        myTable+=  "<td> <button onclick='obtenerItemEspecificoCategory("+items[i].id+")'>Cargar Información</button></td>";
        myTable+=  "<td> <button onclick='editarInformacionCategory("+items[i].id+")'>Actualizar Información</button></td>";
        myTable += "</tr>";
    }
